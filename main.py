...
from database import init_db, save_result, get_all_results, get_result_by_id

app = FastAPI(title="Financial Document Analyzer")

# BONUS: Initialize database on startup — creates tables if they don't exist
init_db()
...
async def analyze_document(
    ...
    try:
        ...
        # BONUS: Save result to database after successful analysis
        save_result(
...
# BONUS: Get all saved analysis results
@app.get("/results")
async def get_results():
    """Retrieve all previously saved analysis results"""
    try:
...
# BONUS: Get a single result by ID
